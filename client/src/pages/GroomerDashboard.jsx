...
const GroomerDashboard = () => {
  ...
  const loadData = async () => {
    try {
      ...
      const mine = res.data.filter(a =>
        a.groomer_id === user.id ||
        a.groomer_name === user.name
      );
  ...
  const totalIngresos = completed.reduce((sum, a) => sum + parseFloat(a.price || 0), 0);
    ...
    <div className="page">
      ...
      <div className="metrics-grid">
        ...
        <div className="metric-card">
          <div className="metric-icon">💰</div>
          <div className="metric-info">
            <span className="metric-value">{formatCOP(totalIngresos)}</span>
